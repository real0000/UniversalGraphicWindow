...
public:
    ...
    ~D12Device() override { if (imm_fence) imm_fence->Release(); if (imm_event) CloseHandle(imm_event); }
    ...
    PipelineHandle create_pipeline(const PipelineDesc& d) override {
        D12Pipeline p; auto* pl = plls_.get(d.layout.id); if (pl) { p.root = pl->root; if (p.root) p.root->AddRef(); }
        ...
        for (int i = 0; i < d.vertex_layout.attribute_count; ++i) { const auto& a = d.vertex_layout.attributes[i]; ie.push_back({ "TEXCOORD", a.location, tex_format(TextureFormat::RGBA8_UNORM), a.buffer_slot, a.offset, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 }); }
        ...
        gd.BlendState.RenderTarget[0].RenderTargetWriteMask = D3D12_COLOR_WRITE_ENABLE_ALL; gd.BlendState.RenderTarget[0].BlendEnable = d.blend.enabled;
        gd.DepthStencilState.DepthEnable = d.depth_stencil.depth_enable; gd.SampleMask = UINT_MAX; gd.SampleDesc.Count = 1;
    ...
    RenderTargetHandle create_render_target(const RenderTargetDesc& d) override { TextureDesc td; td.width = d.width; td.height = d.height; td.format = d.format; td.usage = TEXTURE_USAGE_SAMPLED | TEXTURE_USAGE_RENDER_TARGET; D12RenderTarget rt; rt.color_tex = create_texture(td).id; return { rts_.alloc(rt) }; }
    RenderTargetHandle create_depth_target(const DepthStencilDesc& d) override { TextureDesc td; td.width = d.width; td.height = d.height; td.format = d.format; td.usage = TEXTURE_USAGE_DEPTH_STENCIL; D12RenderTarget rt; rt.depth_tex = create_texture(td).id; return { rts_.alloc(rt) }; }
...
public:
    ...
    void set_render_targets(const RenderTargetHandle*, int, RenderTargetHandle) override { d12_unsupported("set_render_targets (needs RTV descriptor heap)"); }
    ...
    void clear_color(const ClearColor&) override { d12_unsupported("clear_color (needs bound RTV)"); }
    void clear_depth_stencil(const ClearDepthStencil&) override {}
    void set_pipeline(PipelineHandle h) override { auto* p = dev_->pipeline(h.id); if (!p) return; cl_->SetPipelineState(p->pso); if (p->root) { if (p->compute) cl_->SetComputeRootSignature(p->root); else cl_->SetGraphicsRootSignature(p->root); } if (!p->compute) cl_->IASetPrimitiveTopology(p->topo); }
    void bind_vertex_buffer(uint32_t slot, BufferHandle h, uint32_t offset) override { auto* b = dev_->buffer(h.id); if (!b) return; D3D12_VERTEX_BUFFER_VIEW v{ b->res->GetGPUVirtualAddress() + offset, (UINT)(b->size - offset), 0 }; cl_->IASetVertexBuffers(slot, 1, &v); }
    ...
    void blit_render_target(RenderTargetHandle, RenderTargetHandle, int,int,int,int,int,int,int,int, bool) override { d12_unsupported("blit_render_target (draw a fullscreen quad)"); }
